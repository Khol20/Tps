...
public class HommeService implements IDao<Homme> {
    ...
    //Question4
    public List<Femme> getBetweenDates(Homme homme, Date d1, Date d2) {
...
public List<Mariage> getMariagesHommes(Homme homme) {
...
public static void getListEpousesParHomme(Homme h) {
    ...
    try {
        ...
        query.setParameter("hommeId", h.getId());

        List<Femme> epouses = query.list();

        transaction.commit();

        if (!epouses .isEmpty()) {
            System.out.println("Femmes de l'homme dont l'id est: " + h.getId() + ":");
            for (Femme wife : epouses ) {
                System.out.println(wife.getNom() + " " + wife.getPrenom());
            }
        } else {
            System.out.println("Aucune femme trouvée pour l'homme dont l'Id est : " + h.getId());
        }
    } catch (HibernateException e) {
        if (transaction != null) {
 ...
 public void getMarriedWithFourWomenBetweenDates(Date dateDebut, Date dateFin) {
    ...
    private int countMariagesParHomme(int hId, Date dateDebut, Date dateFin) {
        Session session = HibernateUtil.getSessionFactory().openSession();
        Criteria criteria = session.createCriteria(Mariage.class, "m");

        criteria.add(Restrictions.eq("m.homme.id", hId));
        criteria.add(Restrictions.between("m.dateFin", dateDebut, dateFin));

        return ((Number) criteria.setProjection(Projections.rowCount()).uniqueResult()).intValue();
    }
public int countMenMarriedByFourWomenBetweenDates(Date dateDebut, Date dateFin) {
    Session session = null;
    int count = 0;
